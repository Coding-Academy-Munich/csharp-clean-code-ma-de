...
// %% [markdown]
//
// ## Warum Entwurfsprinzipien?
//
// - Software wird häufiger gelesen und geändert als geschrieben
// - Ohne Struktur: wachsende Komplexität, fragiler Code
// - Kleine Änderungen verursachen unerwartete Fehler
// - Entwurfsprinzipien geben bewährte Leitlinien
// - Ergebnis: wartbare, testbare, erweiterbare Systeme

// %% [markdown]
//
// ## Was ist SOLID?
//
// - Akronym für fünf Entwurfsprinzipien
...
// ### SRP-Verletzung
//
// - `OrderProcessor` mischt Berechnung, Persistenz und Benachrichtigung
...
// ### OCP-Verletzung
//
// - Neue Formen erfordern Änderung der `CalculateArea`-Methode
...
// ### OCP eingehalten
//
// - Neue Formen implementieren `IShape`, kein bestehender Code ändert sich
...
// %%
List<IFlyable> flyingBirds = new() { new Sparrow() };
...
// ### DIP-Verletzung
//
// - `UserRegistration` erzeugt `EmailService` direkt
...
class UserRegistrationDip
{
    ...
    public UserRegistrationDip(INotificationService service)
    {
...
//
// - Identifizieren Sie die verletzten SOLID-Prinzipien
// - Begründen Sie, warum es eine Verletzung ist
// - Schreiben Sie eine verbesserte Version, die die Verletzungen behebt
// - Verwenden Sie Interfaces und Dependency Injection
...
// %% [markdown]
//
// - OCP-Verletzung: Neues Log-Ziel erfordert Änderung der `Log`-Methode
// - DIP-Verletzung: `ReportGenerator` erzeugt `Logger` direkt
// - SRP-Verletzung: `ReportGenerator` erzeugt und speichert den Report
...
class FileLogger : ILogger
{
    ...
    public void Log(string message) => File.AppendAllText(filePath, $"[LOG] {message}\n");
}
...
class FileReportStorage : IReportStorage
{
    public void Save(string report) => File.WriteAllText("report.txt", report);
}
...
class ReportService
{
    ...
    public ReportService(ILogger logger, IReportStorage storage)
    {
        this.logger = logger;
